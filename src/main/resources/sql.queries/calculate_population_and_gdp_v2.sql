with validated_input
     ...
     res as (select st.h3, indicator_id, indicator_value
             from boxinput bi
             ...
             where sh.resolution = 8 and indicator_id IN (select internal_id from %s where param_id IN ('gdp','population', 'residential'))),
